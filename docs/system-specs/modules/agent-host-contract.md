...
Five backends are described. **Four are selectable on a plain public build**;
the fifth — OpenCode — is known and install-probed but deliberately NOT
selectable (the section "OpenCode: known, gated, not selectable" below). The
baseline registry `BASELINE_SELECTABLE_BACKENDS` (`acp_backends.py`) therefore
no longer equals `ACP_BACKENDS_KNOWN`; there is no frozen
`ACP_BACKENDS_SELECTABLE` constant, because the set is a registry an
edition may extend and a deployment policy may narrow — and excluding an id
from the baseline is a decision this document records, not an oversight it
hides. Claude Code is not a
...
Two of the selectable four are genuinely *foreign* hosts — **CC and Codex** — and they
...
| **OpenCode** | `ACP_BACKEND_OPENCODE = "opencode"` — the harness's own `opencode acp` stdio subcommand; no adapter package sits between Crew and the binary. **Known and install-probed but NOT selectable on a public build** (§7): its ACP surface exposes no enforceable permission boundary.

### OpenCode: known, gated, not selectable

OpenCode is registered and probed (the install probe reports the binary and the
`opencode auth login` prerequisite), but it is deliberately excluded from
`BASELINE_SELECTABLE_BACKENDS`. The gate is a live probe of `opencode acp`
(v1.18.x) `session/new`: it advertises exactly one config option — a `model`
select — and no permission mode of any shape, so there is nothing to enforce
through `session/set_config_option`. `routing_for("opencode")` therefore
answers `UNVERIFIED` honestly, and because Crew refuses to start a session on
a backend whose tool calls it cannot gate (and deliberately ships no local
opt-out — see `enforce_runtime_routing`), opencode stays non-selectable until
its adapter surface exposes an enforceable permission boundary. A config that
names it falls back to the default backend with a warning, exactly like an
unknown backend in a stock build.

**Auth prerequisite.** opencode authenticates from its own account store, so
the binary alone is not a working backend: after installing, the operator must
run `opencode auth login` before the first turn can be served. The install
probe folds both steps into the one remedy line it reports for a missing
binary (`agent_sdk/backend_install.py`, `_probe_opencode`), and the credential
store is deliberately NOT probed — the checkable paths are not the only way an
opencode authenticates, so a verdict on them would lie in both directions.

## 1. Agent definition and layout

| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---| --- |
| Where a managed agent lives | `~/.kiro/agents/kirocrew.json` (+ `-lite`, `-conductor`, `-knowledge`, `-research`, `-heartbeat` variants, `agent_files.py`) | Nowhere on disk — agents ride `_meta.kiro.customAgents` on `session/new` (`acp/kas_agents.py`) | `~/.claude/agents/<name>.md`, Markdown with YAML frontmatter (companion: a JSON→Markdown agent translator writes it) | Nowhere — spawned bare, with no `--agent` and no spec on disk (`acp/client.py`) | Nowhere — not selectable, so Crew never creates a session on it; whether the harness would read any definition is unmeasured |
| Format | JSON, validated with `deny_unknown_fields`; an unknown key silently falls back to the default agent, which is why bookkeeping lives in an `agent_state` sidecar (`agent.py`) | JSON projected onto the wire; `prompt` must be inlined, and `tools` absent means *no* tools (`kas_agents.py`) | YAML frontmatter (`name/description/model/permissionMode/tools/mcpServers/disallowedTools/hooks`) plus the system prompt as the body | None — no definition is projected in any shape: the spec is not read and no wire equivalent is sent (`acp/client.py`). `providers/mirrors/registry.py` records that as codex's declared state rather than leaving the omission unexplained | unmeasured — no session to project a spec into |
| Selection mechanism | `--agent <name>`; `$PWD/.kiro/agents` shadows `~/.kiro/agents` (`agent_discovery.py`) | `session/set_mode` against a wire-supplied agent | No `--agent` and **no `set_mode` at all**; the agent-activation privilege check is skipped (`acp/client.py`) | No `--agent`, and **no `set_mode`** — activation is gated `if self._is_kiro` (`acp/client.py`). What IS applied before the first prompt is `session/set_config_option("mode", "read-only")`, and a session that never advertised the option is refused (`acp_backends.py`, `acp/client.py`) | no `--agent` is passed and no `set_mode` is sent; moot while non-selectable (`acp/client.py`) |
| Hook event names | Crew's own | Hooks cannot ride an over-the-wire agent (`kas_agents.py`) | Renamed: `PreToolUse` / `PostToolUse` / `UserPromptSubmit` / `SessionStart` / `Stop` (companion) | Crew's own, unrenamed — nothing is projected into the harness, so the gate is reached only through `session/request_permission` → `HookManager.on_tool_call` | unmeasured |
| Model pin | `model` in the spec; `"auto"` resolvable | Not projected | Separate `cc_model` sidecar field; cannot resolve `"auto"`, so background agents get a concrete pin (`agent.py`) | over the wire, not in a spec: `session/set_config_option("model", …)` (`acp_backends.py`). `"auto"` is **not** resolved — it is skipped and the adapter's own default stands (`acp/client.py`). Effort takes the same channel; advertised-spelling folding is excluded (`acp_backends.py`) | the harness advertises a `model` select `configOption` on `session/new` (live probe, v1.18.x) but no Crew channel is wired: excluded from `ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION`, so a pin is withheld rather than sent, and the explicit switch refuses (`acp_backends.py`, `acp/client.py`) |
| Protocol version | date-stamped `2025-08-22` | date-stamped | numeric `1` (`acp/client.py`) | numeric `1`, kept as its own literal rather than folded into CC's (`acp/client.py`) | numeric `1`, kept as its own literal (`PROTOCOL_VERSION_OPENCODE`, `acp/client.py`) |
...
| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---| --- |
| Replay store | `<kiro home>/sessions/cli/<sid>.json` + `.jsonl`, read to resume (`session_storage.py`) | same store (it *is* kiro-cli) | Its own: `<cc root>/projects/<encoded cwd>/<sid>.jsonl` + `<sid>/`, plus `<cc root>/file-history/<sid>/` (companion: a dedicated transcript-cleanup module exists for exactly this) | None on Crew's side — the adapter keeps its own records and resolves them from the `sessionId` (`acp/client.py`); the session map skips both the transcript pre-check and the prune for any non-kiro label (`session_map.py`) | its own records; no Crew-side transcript is kept or consulted — the codex shape applies (`acp/client.py`) |
| Store root | `KIRO_HOME` | `KIRO_HOME` | `CLAUDE_CONFIG_DIR` → `<config dir>/cc-config` → `~/.claude` (companion) | not Crew's to compute — `CODEX_HOME` is read only to re-anchor the credential leaf, never to locate a transcript (`security.py`) | not Crew's to compute |
| Directory naming | session id | session id | `realpath(cwd)` with every non-alphanumeric replaced by `-`. **`realpath`, not `abspath`** — on cloud desktops `/home/<user>` is a symlink to `/local/home/<user>`, and an abspath encoding silently misses the transcript directory (companion) | unknown to this repository — nothing here names or reads a codex session directory | unknown to this repository |
| `session/load` | carries the transcript path, guarded by a file-exists pre-check (`acp/client.py`) | same | carries **no** path; the pre-check is skipped entirely (`acp/client.py`) | carries **no** path and no `_meta`; the file-exists pre-check is skipped, because gating on the kiro transcript would make `file_ok` always False and silently start every resume fresh (`acp/client.py`) | carries **no** path and no `_meta`; the same shape as codex (`acp/client.py`) |
| Compaction | asynchronous, reported by `_kiro.dev/compaction/status`; the session id changes | same | `/compact` runs **synchronously inside `session/prompt`**; no status notification ever arrives, and the session id survives (`dashboard/chat_runner.py`) | not supported — excluded from `ACP_BACKENDS_COMPACT`, so a manual `/compact` is refused up front instead of stranding the status waiter (`acp_backends.py`, `acp/session_provider.py`) | not supported — excluded from `ACP_BACKENDS_COMPACT`, so a manual `/compact` is refused up front (`acp_backends.py`) |
| Sessions per process | many (demultiplexed over `AcpRuntime`) | many | **one** (`acp/types.py`) | **one** — not in `ACP_BACKENDS_ACP_RUNTIME`, so it takes the `AcpClient` path, spawned per session (`acp_backends.py`); no shared subagent session either | **one** — not in `ACP_BACKENDS_ACP_RUNTIME` (`acp_backends.py`) |
| History replay | full history every turn, so one oversized image block wedges a transcript permanently and Crew repairs kiro-cli's own file (`session_image_repair.py`) | same | not applicable | not applicable — no Crew-side transcript to replay or repair | not applicable — no Crew-side transcript |
...
| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---| --- |
| Unit | Kiro credits (`bills_kiro_credits`, membership-only and False for unknown ids) | Kiro credits | US dollars per token | unknown to this repository. `TurnUsage` carries both `credits` and `cost_usd` and each harness fills what it bills in (`acp/types.py`), and the generic `parse_usage_cost` picks up a `cost: {amount, currency}` on `usage_update` from any adapter that sends one, USD only (`acp/_dispatch.py`). Whether codex-acp sends it is not established here | unknown to this repository — no session exists to bill |
...
narrowed, though: with the baseline covering every selectable backend, an edition needs
