...
export default function ReviewFixSetup({
  ...
  const [targetMode, setTargetMode] = useState<ReviewFixTargetMode>('current_branch')
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(190px,0.45fr)]">
        ...
        <label className="flex flex-col gap-1.5 text-[12px] font-semibold text-muted">
          {i18nT('apps.codeReviewSage.reviewFix.target_mode')}
          <SimpleSelect
            aria-label={i18nT('apps.codeReviewSage.reviewFix.target_mode')}
            options={['current_branch', 'new_worktree']}
            optionLabels={[
              i18nT('apps.codeReviewSage.reviewFix.current_branch'),
              i18nT('apps.codeReviewSage.reviewFix.new_worktree'),
            ]}
            value={targetMode}
            onChange={(value) => setTargetMode(value as ReviewFixTargetMode)}
            disabled={create.isPending}
