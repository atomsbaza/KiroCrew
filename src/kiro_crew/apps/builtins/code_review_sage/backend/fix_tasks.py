...
from kiro_crew.dashboard.handlers.taskrunner import _sel
...
async def _validate_sage_findings(
    ...
    if report is None:
        raise ReviewFixPlanError("the Sage review report is unavailable", code="review_report_unavailable")
    rows = report.get("rows")
    if not isinstance(rows, list):
        raise ReviewFixPlanError("the Sage review report is malformed", code="review_report_unavailable")
    matching_rows = [
        row for row in rows
        if isinstance(row, dict) and _text(row.get("url")) == pr_url
    ]
    if len(matching_rows) != 1:
        raise ReviewFixPlanError("the selected pull request is not owned by this review", code="finding_not_owned")
    row = matching_rows[0]
    ...
    if not change_id or not isinstance(row_findings, list):
        raise ReviewFixPlanError("the Sage finding identity is unavailable", code="finding_not_owned")
    if band not in {"red", "yellow"}:
        raise ReviewFixPlanError("only red and yellow findings can be fixed", code="finding_not_eligible")

    seen: set[str] = set()
    for raw in findings:
        if not isinstance(raw, dict):
            raise ReviewFixPlanError("selected finding snapshot is invalid", code="finding_snapshot_mismatch")
        key = raw.get("key")
        if not isinstance(key, str) or key in seen:
            raise ReviewFixPlanError("selected finding identity is invalid", code="finding_not_owned")
        seen.add(key)
        prefix = f"{change_id}:finding:"
        if not key.startswith(prefix) or not key[len(prefix):].isdigit():
            raise ReviewFixPlanError("selected finding is not owned by this pull request", code="finding_not_owned")
        index = int(key[len(prefix):])
        if index < 0 or index >= len(row_findings) or not isinstance(row_findings[index], dict):
            raise ReviewFixPlanError("selected finding is not present in this report", code="finding_not_owned")
        ...
        explicit_only = {
            _text(finding.get(name)).lower()
            for name in ("design_only", "policy_only")
        }
        if (
            kind_values.intersection({"design", "design-only", "policy", "policy-only"})
            or explicit_only.intersection({"1", "true", "yes"})
        ):
            raise ReviewFixPlanError("design-only and policy-only findings cannot be fixed", code="finding_not_eligible")
        ...
        if actual != expected:
            raise ReviewFixPlanError("selected finding snapshot does not match the report", code="finding_snapshot_mismatch")
...
async def handle_create_fix_task(request: web.Request) -> web.Response:
    ...
    try:
        run = await create_review_fix_task(
            ...
            candidate_root=body.get("candidate_root") if isinstance(body.get("candidate_root"), str) else None,
        )
...
async def _apply_group(runner, run, body, expected: int, fingerprint: str):
    ...
    current_target = await review_fix_git.inspect_target(metadata.target.target_path, mode=metadata.target.mode)
    ...
    patch = await review_fix_git.write_patch(
        patch,
        Path(metadata.git.candidate_worktree_path) / ".kirocrew-review-fix-artifacts" / f"{group_id}.patch",
    )
    await review_fix_git.apply_patch(current_target, patch)
    applied_target = await review_fix_git.inspect_target(metadata.target.target_path, mode=metadata.target.mode)
...
async def _commit_group(runner, run, body, expected: int, fingerprint: str):
    ...
    committed_target = await review_fix_git.inspect_target(metadata.target.target_path, mode=metadata.target.mode)
    ...
    return await runner.mutate_review_fix(
        ...
        group_id=group_id,
        action="commit_group",
        to_state=ReviewFixState.COMMITTED,
        mutate=mutate,
    )
...
async def _action(
    ...
    if action == "edit_soft_grouping":
        ...
        if any({new_by_key[key].group_id for key in keys} != {new_by_key[next(iter(keys))].group_id} for keys in old_hard):
    ...
    if action == "reevaluate_grouping":
        return await runner.mutate_review_fix(
            run.task_id,
            expected_revision=expected,
            expected_target_fingerprint=fingerprint,
            action=action,
            mutate=lambda current: current.logs.append("grouping re-evaluation requested"),
        )
    ...
    if action in {"replan_preview", "confirm_replan"}:
        if action == "confirm_replan":
            replacement = body.get("groups")
            if not isinstance(replacement, list):
                raise ValueError("groups is required")
            groups = build_review_fix_groups(metadata.finding_snapshots, replacement)
        ...
        return await runner.mutate_review_fix(
            ...
            expected_target_fingerprint=fingerprint,
            action=action,
            mutate=lambda current: current.logs.append("re-plan preview requested"),
        )
    ...
    if action == "validate_group":
        ...
        if not isinstance(test_command, list) or not test_command or not all(
            isinstance(value, str) for value in test_command
        ):
        ...
        if not isinstance(build_command, list) or not build_command or not all(
            isinstance(value, str) for value in build_command
        ):
            raise ValueError("test_command and build_command are required")
    ...
    if action == "push_preview":
        if metadata.state not in {ReviewFixState.COMMITTED, ReviewFixState.AWAITING_PUSH}:
            raise ValueError("task is not ready for push preview")
    ...
    if action == "recheck_continue":
        if metadata.state not in {
            ReviewFixState.BLOCKED_TARGET_CHANGED,
            ReviewFixState.BLOCKED_CONFLICT,
            ReviewFixState.BLOCKED_MANUAL_RESOLUTION,
            ReviewFixState.BLOCKED_VALIDATION,
        }:
            raise ValueError("task does not require a re-check")
        current = await review_fix_git.inspect_target(metadata.target.target_path, mode=metadata.target.mode)
        return await runner.mutate_review_fix(
            run.task_id,
            expected_revision=expected,
            expected_target_fingerprint=fingerprint,
            ...
            mutate=lambda current_metadata: setattr(current_metadata, "target", current),
        )
    if action == "discard_candidate":
        await review_fix_git.discard_candidate(metadata.git, metadata.target.repo_root)
        return await runner.mutate_review_fix(
            run.task_id,
            expected_revision=expected,
            expected_target_fingerprint=fingerprint,
...
async def handle_fix_action(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if isinstance(result, str):
            ...
            return web.json_response({"ok": True, **_payload(run), "execution_task_id": result}, status=202)
...
async def handle_review_again(
    ...
    try:
        ...
        if not isinstance(expected, int) or not isinstance(fingerprint, str):
            return _error("review_again_context_required", "expected_revision and target_fingerprint are required")
        if run.review_fix is None or run.review_fix.state not in {
            ReviewFixState.PUSHED,
            ReviewFixState.READY_TO_REREVIEW,
        }:
        ...
        run = await runner.mutate_review_fix(
            ...
            expected_revision=expected,
            expected_target_fingerprint=fingerprint,
            expected_state=run.review_fix.state,
            action="review_again",
            to_state=ReviewFixState.REREVIEWING,
            mutate=lambda current: None,
        )
        if review_again_handler is None:
            return web.json_response({"ok": True, **_payload(run), "review_run_id": ""}, status=202)
        forwarded = dict(body)
...
def register_fix_task_routes(
    app: web.Application,
    review_again_handler: Callable[[web.Request, dict[str, Any]], Awaitable[web.Response]] | None = None,
    *,
    include_taskrunner: bool = True,
) -> None:
    """Register Sage fix-task endpoints and optionally Task Runner endpoints."""
    if include_taskrunner:
        app.router.add_get("/api/taskrunner/{task_id}/review-fix", handle_get_fix_task)
        app.router.add_post("/api/taskrunner/{task_id}/review-fix/actions", handle_fix_action)
