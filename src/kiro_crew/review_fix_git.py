...
async def _git_text_bounded(work_dir: str | Path, cap_bytes: int, *args: str) -> str:
    ...
    Same sandboxed spawn as ``git_coord._git``, but the output is drained
    through ``drain_bounded`` so the gateway never buffers more than
    *cap_bytes* regardless of what the diff contains. Truncation FAILS the
    call instead of returning a short result — callers capture patches and
    owned-path lists where a silent truncation would corrupt the apply/
    ownership contract.
    """
    ...
    try:
        ...
        out, err = await asyncio.gather(
            ...
            drain_bounded(proc.stderr, _STDERR_CAP_BYTES),
        )
    ...
    if returncode != 0:
        message = err[0].decode(errors="replace").strip() or f"git {' '.join(args)} failed"
        message = redact_credentials(message)[0]
        message = redact_exfiltration_urls(message)[0]
    ...
    return out[0].decode(errors="replace")
