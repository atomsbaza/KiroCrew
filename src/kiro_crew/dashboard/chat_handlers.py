...
_AUTOMATIC_ROUTE_BLOCKED = "Automatic Crew routing is unavailable. No work was started; retry."
...
async def _automatic_route_ack(
    request: web.Request, *, slot: _ChatSlot, kind: str
) -> web.StreamResponse:
...
async def _try_automatic_route(
    ...
    *,
    ...
    if service is None:
        ...
        return await _automatic_route_blocked(request, state, slot, pairing_task_id=pairing_task_id)
    try:
...
async def api_chat(request: web.Request) -> web.StreamResponse:
    ...
    _skip_automatic_route = bool(
        _pairing_active is not None and _pairing_active.get("mode") in {"guided", "practice"}
    )
    ...
    if _automatic_route_eligible(message, agent, slot) and not body.get("steer"):
        if _pairing_pending is None and _pairing_active is None:
            ...
            if eligibility.eligible:
                ...
                return await _automatic_route_ack(request, slot=slot, kind="pairing_preflight")
