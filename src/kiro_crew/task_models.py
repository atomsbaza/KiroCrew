...
class ReviewFixState(str, enum.Enum):
    ...
    PUSHED = "pushed"
    READY_TO_REREVIEW = "ready_to_rereview"
    ...
    BLOCKED_TARGET_CHANGED = "blocked_target_changed"
    BLOCKED_CONFLICT = "blocked_conflict"
    BLOCKED_MANUAL_RESOLUTION = "blocked_manual_resolution"
...
class ReviewFixTargetSnapshot:
    ...
    def from_dict(cls, raw: Mapping[str, Any] | None) -> "ReviewFixTargetSnapshot":
        raw = raw or {}
        try:
            mode = ReviewFixTargetMode(str(raw.get("mode", ReviewFixTargetMode.CURRENT_BRANCH.value)))
        except ValueError:
        ...
        return cls(
            ...
            tracked_paths=[str(path) for path in raw.get("tracked_paths", []) if isinstance(path, str)],
            untracked_paths=[str(path) for path in raw.get("untracked_paths", []) if isinstance(path, str)],
    ...
    def from_dict(cls, raw: Mapping[str, Any] | None) -> "ReviewFixModelResolution":
        ...
        return cls(
            ...
            advertised_model_ids=[
                str(model) for model in raw.get("advertised_model_ids", []) if isinstance(model, str)
            ],
...
class ReviewFixDependencyGroup:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "ReviewFixDependencyGroup":
        ...
        return cls(
            ...
            hard_edges=[dict(edge) for edge in raw.get("hard_edges", []) if isinstance(edge, Mapping)],
            soft_edges=[dict(edge) for edge in raw.get("soft_edges", []) if isinstance(edge, Mapping)],
...
class ReviewFixGitRecord:
    ...
    def from_dict(cls, raw: Mapping[str, Any] | None) -> "ReviewFixGitRecord":
        ...
        return cls(
            ...
            remote=str(raw.get("remote", "")),
            upstream=str(raw.get("upstream", "")),
            push_preview=dict(raw.get("push_preview", {})) if isinstance(raw.get("push_preview"), Mapping) else {},
            push_result=dict(raw.get("push_result", {})) if isinstance(raw.get("push_result"), Mapping) else {},
...
# Friendly aliases keep the domain names discoverable for adapters/tests.
ReviewFindingSnapshot = ReviewFixFindingSnapshot
ReviewFixTarget = ReviewFixTargetSnapshot
ReviewFixGroup = ReviewFixDependencyGroup
ReviewFixValidation = ReviewFixValidationRun
...
class Project:
    ...
    review_fix: ReviewFixMetadata | None = None  # optional review-fix state; generic runs leave this absent
