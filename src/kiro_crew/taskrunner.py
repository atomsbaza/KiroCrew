...
class ReviewFixConflict(ValueError):
    ...
    def __init__(self, run: Project, reason: str) -> None:
        ...
        self.current_group_revisions = {
            group.group_id: group.revision for group in (run.review_fix.groups if run.review_fix else [])
        }
...
_REVIEW_FIX_TRANSITIONS: dict[ReviewFixState, set[ReviewFixState]] = {
    ...
    ReviewFixState.AWAITING_GROUP_CONFIRMATION: {
        ...
        ReviewFixState.BLOCKED_DEPENDENCY,
        ReviewFixState.PLANNING,
        ReviewFixState.FAILED,
    },
    ReviewFixState.RUNNING: {
    ...
    ReviewFixState.READY_TO_APPLY: {
        ReviewFixState.AWAITING_COMMIT,
        ReviewFixState.APPLIED,
        ReviewFixState.BLOCKED_TARGET_CHANGED,
        ReviewFixState.BLOCKED_CONFLICT,
    },
    ReviewFixState.APPLIED: {ReviewFixState.AWAITING_COMMIT, ReviewFixState.BLOCKED_CONFLICT},
    ReviewFixState.AWAITING_COMMIT: {ReviewFixState.COMMITTED},
    ReviewFixState.COMMITTED: {ReviewFixState.AWAITING_PUSH},
    ReviewFixState.AWAITING_PUSH: {ReviewFixState.PUSHED},
    ReviewFixState.PUSHED: {ReviewFixState.READY_TO_REREVIEW, ReviewFixState.REREVIEWING},
    ReviewFixState.READY_TO_REREVIEW: {ReviewFixState.REREVIEWING, ReviewFixState.DONE},
    ReviewFixState.REREVIEWING: {ReviewFixState.DONE, ReviewFixState.FAILED},
    ReviewFixState.BLOCKED_MODEL_RESOLUTION: {
        ReviewFixState.PLANNING,
        ReviewFixState.AWAITING_GROUP_CONFIRMATION,
        ReviewFixState.RUNNING,
    },
    ReviewFixState.BLOCKED_DIRTY_OVERLAP: {ReviewFixState.PLANNING},
    ReviewFixState.BLOCKED_DEPENDENCY: {ReviewFixState.AWAITING_GROUP_CONFIRMATION, ReviewFixState.PLANNING},
    ReviewFixState.BLOCKED_VALIDATION: {ReviewFixState.RUNNING, ReviewFixState.AWAITING_VALIDATION},
    ReviewFixState.BLOCKED_TARGET_CHANGED: {ReviewFixState.AWAITING_VALIDATION, ReviewFixState.PLANNING},
    ReviewFixState.BLOCKED_CONFLICT: {ReviewFixState.BLOCKED_MANUAL_RESOLUTION},
    ReviewFixState.BLOCKED_MANUAL_RESOLUTION: {ReviewFixState.AWAITING_VALIDATION},
    ReviewFixState.PAUSED: {ReviewFixState.RUNNING},
    ReviewFixState.FAILED: {ReviewFixState.RUNNING, ReviewFixState.PLANNING},
    ...
    async def mutate_review_fix(
        ...
        *,
        ...
        if expected_state is not None:
            expected_value = (
                expected_state.value if isinstance(expected_state, ReviewFixState) else str(expected_state)
            )
            if metadata.state.value != expected_value:
                raise ReviewFixConflict(run, "task state is stale")
        if expected_target_fingerprint and metadata.target.dirty_fingerprint != expected_target_fingerprint:
        ...
        if next_state != metadata.state and next_state not in _REVIEW_FIX_TRANSITIONS[metadata.state]:
            raise ValueError(f"Invalid review-fix transition: {metadata.state.value} -> {next_state.value}")
        ...
        try:
            for item in items:
                ...
                run = Project(
                    ...
                    revision=int(item.get("revision", review_fix.revision if review_fix else 0) or 0),
                    execution_mode=item.get("execution_mode", "review_fix" if review_fix else "standard"),
                    commit_policy=item.get("commit_policy", "manual_group" if review_fix else "per_task"),
                )
